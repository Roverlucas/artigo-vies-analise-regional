...
\usepackage{siunitx}
\sisetup{group-separator={.}, group-minimum-digits=4, range-phrase={ a }}
...
Confiabilidade da informação regulatória que os LLMs recordam para a gestão
...
\input{sections-PT/01_introducao}
\input{sections-PT/02_trabalhos_relacionados}
\input{sections-PT/03_metodos}
\input{sections-PT/04_resultados}
\input{sections-PT/05_discussao}
\input{sections-PT/06_conclusao}
...
as respostas completas dos modelos serão liberados na publicação em
\href{https://github.com/Roverlucas/artigo-vies-analise-regional}{github.com/Roverlucas/artigo-vies-analise-regional},
sob licenças MIT (código) e CC-BY-4.0 (dados, prompts, documentação). A liberação
...
com \texttt{python code/run\_all.py --confirmatory}. Concretamente, o diretório
que o pipeline lê como \texttt{data/confirmatory\_PRIVATE/} é publicado inteiro
sob o nome \texttt{data/responses/}: os mesmos arquivos, renomeados, sem nada
removido. Nenhuma redação é necessária e nenhuma é aplicada, porque os registros
contêm saídas de modelos a perguntas públicas de política e nenhum dado de
sujeito humano; pela mesma razão não os descrevemos como anonimizados. O nome do
flag \texttt{--confirmatory} vem do protocolo fechado.
...
e Claude Haiku~4.5 pontuaram respostas contra o registro de gabarito como juízes
da coleta; Gemini~2.5~Pro, Claude Sonnet~4.6 e DeepSeek-V3 as repontuaram como
painel de três fornecedores; e o Claude Sonnet~4.6 produziu as versões dos prompts
em idioma nativo. Esses usos fazem parte do método, estão descritos na
...
autores usaram o Claude, da Anthropic (via Claude Code), para auxiliar na redação
e edição do texto e na implementação do código de análise; o Codex CLI, da
OpenAI, para rodar uma auditoria adversarial do manuscrito, do suplemento e do
código contra suas fontes; e, por API, o Gemini~2.5~Pro e o DeepSeek-V3 para
garantia de qualidade interna das afirmações e da prosa do manuscrito, rodados em
fornecedor diferente do que redigiu o trecho revisado. Uma execução equivalente
no GPT-5.2-pro foi tentada e nunca aconteceu, porque a conta estava sem crédito;
a tentativa falha está nos artefatos liberados e nenhuma saída desse modelo está
neste artigo. São passagens de garantia de qualidade, não revisão por pares: todo
achado que voltou foi conferido na fonte antes de qualquer edição, e os que
rejeitamos estão registrados com o motivo. Após
usar essas ferramentas, os autores
...
Os autores declaram não haver conflitos de interesse. Os modelos aqui avaliados
foram acessados por camadas de API pagas ou gratuitas comuns; nenhum fornecedor
financiou, revisou ou teve qualquer conhecimento deste estudo, e nenhum teve
papel no desenho, na análise ou na decisão de publicar.
