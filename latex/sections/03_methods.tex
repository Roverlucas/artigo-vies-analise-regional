...
Scoring assigns each task the most reliable instrument it admits, which means
the paper carries more than one scoring regime at once.
Table~\ref{tab:scoring} is the whole architecture on one page; the paragraphs
that follow give the reasons behind it.

\begin{table}[h]
\centering
\caption{Who decides what, per task. ``Factual verdict'' is the subcomponent
carrying weight $0.30$ in the composite; the other four subcomponents are always
a judge's. The primary tier outcome uses only the code column.}
\label{tab:scoring}
\scriptsize
...
\toprule
Task & The answer is & Factual verdict & Other four subcomponents & In primary outcome \\
\midrule
T1 & a national value        & code, against the register & collection judge & yes \\
T2 & a national value        & code where extractable, else panel & panel or collection judge & yes \\
T3 & an international estimate & code where extractable, else panel & panel or collection judge & yes \\
T4 & national instruments    & three-vendor panel & three-vendor panel & no \\
T5 & no register value       & collection judge & collection judge & no \\
\bottomrule
\end{tabular}
\end{table}
...
The composite on T1 therefore still carries, in $0.70$ of its weight, the
judge-and-tier confound described below. That is why the primary outcome for the
tier comparison is the binary code verdict itself, whether the answer returned
the value in force, with the composite as the secondary outcome
(Section~\ref{sec:results:h1}).
