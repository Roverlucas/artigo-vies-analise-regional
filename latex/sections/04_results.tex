...
single fact. Pooling the two recall tasks (T1, T2; mean $0.434$) against
...
the wrong T1 answers (281 of 759) cite a value that sits on the country's official
...
points (Wilcoxon signed-rank $p=5\times10^{-16}$). That test treats cells as
...
differences with a random intercept per country gives $-5.0$~pp
(SE $1.0$, $p=6\times10^{-7}$), with a random intercept per model $-4.8$~pp
($p=3\times10^{-9}$); with the country as the unit ($n=9$) the penalty is
negative in eight of nine and averages $-5.1\pm1.1$~pp ($t=-4.6$, $p=0.002$);
with the model as the unit ($n=14$) it is negative in twelve of fourteen
(Wilcoxon $p=0.058$). The penalty tracks the resource level of the language,
and all three are significant (Table~\ref{tab:h2}): Hindi $-11.4$~pp, Spanish
$-4.5$~pp, Portuguese $-4.2$~pp. India, which scores highest
...
best-resourced of the three, shows no such loss ($+3.5$~pp). Portuguese
...
code in both languages) the Portuguese penalty is $-9.1$~pp ($n=132$, $p=0.019$)
and the Spanish one $-2.1$~points ($n=165$, not significant), $-5.0$~pp overall
($n=318$, $p=0.048$). Asking in the local language does not merely lower a
...
whose 95\% bootstrap confidence interval over countries is $[+1.6,+8.4]$~pp and
\emph{excludes zero} (permutation of the tier label, country as unit,
$p=0.019$). It is stable to dropping any single country (leave-one-out
$[+4.4,+6.0]$~pp) and to the composite weighting ($+4.2$~pp under equal weights,
$+8.7$~pp on the factual subcomponent, where it widens). It does not depend on
...
return the register value $0.61$ of the time and Global South countries
$0.46$, a gap of $+14.7$~pp $[+5.6,+23.9]$ (permutation $p=0.012$), and the
pooled odds ratio is $0.53$ $[0.48,0.58]$. Whatever the panel adds on the judged
...
against Colombia, Chile and Angola) it is $+5.9$~pp $[+2.4,+9.6]$, and on the
pre-specified fifteen (three Global North against twelve) $+6.0$~pp
$[+1.8,+10.3]$, a point estimate of the same size on a sample too small to make
...
Global South odds of returning the register value are $0.32$ of the Global
North's ($50.7\%$ versus $30.9\%$ raw), and on policy instruments (T4)
...
countries with no national standard it is $0.39$ $[0.32,0.49]$, and on the
pre-specified fifteen it is $0.27$ $[0.21,0.34]$. T4 is the one task in the
...
\midrule
T1 (technical standard)     & a national value        & 50.7\% & 30.9\% & 0.32 & $[0.27,0.39]$ \\
...
under equal weights it is $0.36$ and under PCA weights $0.38$. We
...
Closed-accessible models average $+12.8$~pp above open-weight ones, against the
...
aside and the Global South odds on T1 are $0.39$ $[0.32,0.49]$ of the Global
North's, against $0.32$ with them. The key also moved under review, in the
...
$0.30$, because most models cite the superseded value. The full registry is in
...
return 9.0 for the United States. Of the 759 wrong T1 answers, 281 cite a value
...
and Global South accuracy only from $0.31$ to $0.42$, because the staged
instruments are mostly in the North, and the odds ratio falls to $0.14$
$[0.11,0.16]$. Where the register offers several plausible numbers the models
...
the independent unit. Chance clears the observed gap $1.9\%$ of the time
($p=0.019$). A Bayesian hierarchical model with crossed random intercepts agrees
($\beta=-0.049$, 94\% HDI $[-0.088,-0.006]$, $P(\beta<0)=0.987$), while a
frequentist mixed model returns the same magnitude and sign but leaves it marginal
($\beta=-0.063$, $p=0.086$), because it charges the large between-country
...
an E-value of $1.71$ at the point estimate and $1.31$ at the confidence limit
...
H1 tier gap        & $+5.0$~pp; OR $0.32$ on T1  & tier \\
\midrule
...
\addlinespace[2pt]
H5 open/closed     & $+12.8$~pp for closed       & access type \\
\bottomrule
\end{tabular}
