...
language lowers accuracy by $5.0$~pp, significantly in all three languages, and
...
The local-manager persona does not narrow the gap (DiD $+0.5$~pp, permutation
...
($\delta=-0.48$), and it also trails its scale-matched peer on the
Portuguese-language Brazilian prompts it was tuned for ($0.258$ against $0.313$
...
buying back the local knowledge it promises; the narrow test is small, and we
weight the claim accordingly.
...
$0.435$ for normative and local-datum recall against $0.612$ for synthesis and
recommendation ($\delta=-0.41$), deepest on the national standard itself
($0.394$). The risk falls unevenly: a manager drafting prose about air quality
...
in a lower-resource language. The floor is genuine rather than an artefact of
scoring: both recall tasks are now decided by code against the register, T2
rose from $0.368$ to $0.473$ when its placeholder key was rebuilt, and T1, once
moved from two collection judges of different severity to code, stays the hardest
task at $0.394$, with a third of its wrong answers citing a value from the
country's own official ladder rather than an invented one.
...
Read on the composite the gap looks modest ($+5.0$~pp, bootstrap CI
$[+1.5,+8.4]$), and it is the same size inside the extension wave alone and on
the pre-specified fifteen, so the wave that collected it does not explain it.
Estimated task by task on the binary outcome of returning the register value, it
is not modest: the gap scales with how much the answer depends on a fact specific
to that country, from an odds ratio of $0.33$ on the binding national standard
--- $0.39$ once the countries with no or unconfirmed standard are set aside ---
to no gap at all on open recommendation, the one task with no register value to
get right. The disadvantage is concentrated, and averaging five tasks into one
composite dilutes it with tasks where it does not exist. On the question a
manager actually brings to these systems --- what is the standard in force here
--- a Global South country is served at a third of the odds of a Global North
one. That is the faithful statement of the geographic finding.

The gradient stays below the pre-specified threshold on every reading, and the null is informative rather than empty
...
($\beta=+0.024$, $p=0.003$): what that coefficient measures cannot be language.
What it can be is development. Entered together with coverage, HDI$\times$task
dependence takes the signal ($\beta=+0.027$, $p=5\times10^{-5}$) and coverage
falls to $\beta=+0.012$ ($p=0.063$), because across these 25 countries the two
correlate at $\rho=0.65$ and the design has no lever that moves one without the
...
registers and moving the comparison into code cuts the gradient to $\rho=0.36$,
below the pre-specified threshold, more than doubles the language penalty to
$-5.0$~pp, and converts Spanish from a null into a significant harm. Moving the
third register-valued task, T1, from a judge to code changed the headline odds
ratio on that task from $0.22$ to $0.33$, because the two collection judges had
been split along the same line as the tiers. The dataset did not change; only the
