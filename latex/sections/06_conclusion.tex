...
The remedies an agency can try do not work, and the most intuitive one
...
represented in training corpora; a local-manager persona does not narrow the
gap and is equivalent to no persona within 2~pp; and the regionally fine-tuned
7B model is the weakest of all fourteen systems. Accuracy is also lowest on the
recall of a binding value, the task a manager can least afford to have wrong. A
Global North/South tier gap persists and is structured: on the outcomes no
judge touches, Global South countries return the register value
$\ncodegap$~pp less often, and the gap shrinks as the answer depends less on a
country-specific fact, disappearing on the one task with no register value to
get right. The development gradient does not reach significance and its
interval includes both zero and the pre-specified threshold, so we report a
difference between groups that we can demonstrate and a shape across the
development range that we cannot. On mechanism, the within-country design
points toward country coverage and does not establish it once the country is
the unit of inference, and we claim no more than that.
