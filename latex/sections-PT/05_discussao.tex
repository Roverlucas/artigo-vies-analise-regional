...
próprio país reduz o composto em $\nhtwomixedcountry$~pp com intercepto aleatório
por país, significativamente nos três idiomas e com o país como unidade de
inferência, e a maior penalidade recai
...
A persona de gestor local equivale a nenhuma persona. A
diferença-em-diferenças é $\ndid$~pp (permutação $p=\npersonap$) e seu intervalo
por país cabe dentro de uma margem de $\pm2$~pp, de modo que este é um resultado
de equivalência e não um nulo sem poder, em consonância com a
...
O modelo regional não compra nada em relação ao modelo de que partiu. Nos
valores de registro, reservidos nos mesmos itens em português, o Cabra devolve o
valor em vigor em \ncabsanitycabra\% das vezes e seu modelo-base em
\ncabsanitybase\%: o ajuste de instrução em português brasileiro não melhorou o
que o modelo recorda da regulação brasileira. Ele também fica em último entre os
catorze e atrás do seu par de escala nos prompts em português para os quais foi
ajustado ($\nhthreebra$ contra $\nhthreebrallama$ do Llama~3.1~8B;
$\nhthreenarrow$~pp nos 30 prompts em português), mas essas comparações correm
sobre o composto, cujos subcomponentes julgados vêm dos juízes da coleta, e a
posição na lista compara um modelo de 7B com sistemas uma ordem de grandeza
maiores. A afirmação defensável é a estreita, e é a que um órgão precisa: baixar
um modelo pequeno com ajuste regional não recupera os fatos nacionais que um
modelo maior erra. Dois limites a cercam: o teste estreito tem 30 itens por
modelo, e o modelo que pudemos rodar é um ajuste comunitário de uma base de 7B,
não um modelo regional comercial de fronteira do tipo que um órgão poderia
adquirir. Para esses, o achado é uma previsão, e o protocolo liberado é o modo
de testá-la.

Os praticantes implantam os três como correções. Um causa dano ativo, um
equivale a não fazer nada, e um não acrescenta nada ao modelo de que partiu. Para órgãos
...
Mudar a unidade de variação compra um resultado e recusa outro. No nível do
país, o déficit nas tarefas específicas do país é previsto pelo desenvolvimento
($\beta=\nhfourhdialonebeta$, $p=\nhfourhdialoneclp$ agrupado;
$\rho=\nhfourctryrhohdi$, $p=\nhfourctryphdi$ nos 25 déficits) e não, em níveis
convencionais, pela cobertura enciclopédica ($p=\nhfourclp$;
$\rho=\nhfourctryrho$, $p=\nhfourctryp$). As duas se correlacionam a
$\rho=\nrhocobhdi$ e o desenho não tem alavanca que mova uma sem a outra, de
modo que isso não identifica o desenvolvimento como o canal operante; diz que a
exposição que supera o limiar é justamente a que o mecanismo pré-especificado
pretendia descartar.

O resultado também é mais estreito do que o gradiente com que se parece. O
desenvolvimento não prevê com que acurácia os modelos respondem sobre um país
(Seção~\ref{sec:res:h1}); prevê o quanto respondem pior às perguntas cuja
resposta é um fato daquele país. São desfechos diferentes, e só o segundo tem
associação no nível do país. O que continua excluído é o canal de corpus do
idioma, como tem de ser para um contraste medido inteiramente em inglês.
...
especialistas humanos de uma amostra estratificada converteria um resultado
ancorado em fonte oficial num resultado ancorado em padrão-ouro. A amostra é
exportada com a liberação (\texttt{code/analysis/export\_human\_validation.py}):
150 respostas estratificadas por tier, idioma e tarefa, três planilhas com o
prompt, a resposta e o gabarito oficial, mas sem o modelo, o tier e a nota da
máquina, e um arquivo-chave aberto só depois que as planilhas voltarem.
...
As três correções a que os praticantes recorrem primeiro falham juntas, cada
uma à sua maneira: o idioma local prejudica, o enquadramento de persona equivale
a não usar enquadramento, e o ajuste regional em 7B não acrescenta nada ao
modelo-base de que partiu. O resultado metodológico é que, nesta literatura, a
chave de gabarito e o nível em que a inferência é feita importam mais que o
tamanho da amostra; mudar esses dois, sem tocar em uma única resposta, levou um
gradiente de desenvolvimento significativo a um nulo, dobrou a penalidade de
idioma e transformou um mecanismo de corpus estabelecido em questão aberta. E o
desenho separa o que 25 países conseguem demonstrar do que não conseguem,
relatando o segundo conjunto como aberto e não como sustentado.
