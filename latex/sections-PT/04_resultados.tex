...
Agrupando as duas tarefas de recuperação (T1, T2; média 0.435) contra síntese e
...
veredito de código também mostra o que os juízes não podiam: um terço das
respostas erradas de T1 (259 de 754) cita um valor que está na escada oficial do
...
$p=5\times10^{-16}$). A penalidade acompanha o nível de recurso do idioma, e as
...
português, o mais bem provido dos três, não mostra tal perda ($+3.5$~pp).
Perguntar no idioma local não apenas reduz uma nota julgada; onde o idioma é
escasso, torna o modelo substancialmente menos propenso a devolver um número
conferível.
...
intervalo de confiança bootstrap de 95\% entre países é $[+1.5,+8.4]$~pp e
\emph{exclui o zero} (permutação do rótulo de camada, país como unidade,
$p=0.021$). Ela é estável à remoção de qualquer país isolado
(leave-one-out $[+4.4,+5.9]$~pp) e à ponderação do composto ($+4.2$~pp sob pesos
iguais, $+8.7$~pp apenas no subcomponente factual, onde se amplia). Como sete dos
...
Colômbia, Chile e Angola) ela é de $+5.9$~pp $[+2.3,+9.6]$, e nos quinze
pré-especificados (três do Norte contra doze) de $+6.0$~pp $[+1.7,+10.2]$, uma
...
o Sul Global devolver o valor de registro é 0.33 da chance do Norte (50.7\%
contra 31.6\% em taxa bruta), e nos instrumentos de política (T4) também 0.33;
...
(Seção~\ref{sec:res:t1}): deixando de lado os quatro países do Sul Global cuja
chave não é um valor nacional plenamente verificado ela é 0.39 $[0.31,0.50]$, e
nos quinze pré-especificados é 0.27 $[0.21,0.34]$. A lacuna não é fraca --- ela
...
\midrule
T1 (padrão técnico)          & um valor nacional      & 50.7\% & 31.6\% & 0.33 & $[0.27,0.40]$ \\
...
(faixa $[+0.30,+0.48]$); sob pesos iguais é $0.35$ e sob pesos de ACP $0.37$. Relatamos, portanto, uma diferença de camada que
...
Modelos fechados acessíveis têm média $+13.0$~pp acima dos de pesos abertos, contra
...
fiel deve se recusar a declarar um limiar inexistente. Duas chaves não estão
plenamente verificadas (Bangladesh, pontuado contra seu diário oficial de 2005;
Egito, sem valor recuperável e portanto sem chave de T1), e todos esses cinco
casos são do Sul Global, de modo que a composição da chave é ela própria uma
explicação candidata para a lacuna de T1. Não é: deixando-os de lado, a chance do
Sul Global em T1 é 0.39 $[0.31,0.50]$ da do Norte, contra 0.33 com eles. O
...
Unidos passaram de 12,0 para 9,0 em 2024. A chave de T1 é o valor em vigor na
...
devolve 9,0 para os Estados Unidos. Das 754 respostas erradas de T1, 259 citam um
...
a do Sul Global apenas de 0.32 para 0.40, porque os instrumentos escalonados
estão em sua maioria no Norte, e a razão de chances cai para 0.12
$[0.10,0.15]$. Onde o registro oferece vários números plausíveis os modelos os
...
resposta, como unidade independente. O acaso supera a lacuna observada em 2.1\% das
vezes ($p=0.021$). Um modelo hierárquico bayesiano com interceptos aleatórios
cruzados concorda ($\beta=-0.048$, HDI de 94\% $[-0.090,-0.009]$,
$P(\beta<0)=0.987$), enquanto um modelo misto frequentista devolve a mesma magnitude
e sinal mas o deixa marginal ($\beta=-0.063$, $p=0.087$), porque cobra do termo de
...
sensível à especificação, com E-value de 1.70 na estimativa pontual e 1.30 no limite
...
H1 lacuna de camada & $+5.0$~pp; RC 0.33 em T1     & camada \\
\midrule
...
\addlinespace[2pt]
H5 aberto/fechado   & $+13.0$~pp para fechados     & tipo de acesso \\
\bottomrule
\end{tabular}
