...
| 5.1 | Pilot Calibration Study (Pilot 1 + Pilot 2.0 com LLM-as-judge) | `experiment-runner (Forge)` | ✅ Concluída | `data/pilot_202604/`, `data/pilot_v2/` | 2026-04-25 |
| 5.2 | OSF Pre-Registration v6 (forward-looking) — aguardando aprovação Yara/Eduardo | `submission-engineer (Atlas)` | 🟡 Aguardando deposit | `preregistration/osf_prereg_draft.md` (+ PDF) | 2026-04-26 |
| 5.3 | Execução Confirmatória (15 países × 14 modelos × 2 reps = 12.600 calls) | `experiment-runner (Forge)` | 🔄 Em andamento (BRA ✅ 839/840) | `data/confirmatory_PRIVATE/` (gitignored) | 2026-04-28 |
...
- **2026-04-25:** Pilot 1 + Pilot 2.0 (LLM-as-judge ensemble) finalizados. Identificado e corrigido bug GPT-5 (`reasoning_effort='minimal'` + `max_completion_tokens`), bug Qwen3 thinking-mode (think=false / reasoning_effort='none'), bug Sabiá-7B (base model → substituído por Cabra-Mistral 7B). Citation Verification Protocol (CVP) instituído após detecção de fabricação de citações em v3.0; project-auditor + citation-sentinel-v2 criados.
- **2026-04-26:** Pre-registro v6 finalizado (forward-looking proposal, sem menção de erros internos), incorporando feedback de 4 rodadas de auditoria (P0+P1). Reserve model trocado para Gemini 2.5 Pro (evita viés óptico de vendor budget). H1 canônico = Spearman ρ ≥ 0.55 a nível de país (n=15); GLMM apenas para variance partitioning. PCA com loadings derivadas do pilot (sem double-dipping). Anonimização stylistic-only (sem paraphrasing). Confirmatório iniciado em paralelo ao depósito OSF — Brasil primeiro.
- **2026-04-28:** Coleta confirmatória **BRA finalizada com 839/840 (99.88%)** — 14 modelos × 30 prompts × 2 reps. 1 erro transiente irrecuperável em qwen3_32b. Próximo: autorar 30 prompts confirmatórios para os 14 países restantes (USA, IND, NGA, MEX, ARG, PER, ZAF, KEN, EGY, IDN, BGD, PHL, DEU, JPN). Dados em `data/confirmatory_PRIVATE/` (gitignored conforme §3.6 #4 do pré-registro).
