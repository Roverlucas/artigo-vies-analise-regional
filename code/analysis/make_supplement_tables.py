...
Inputs (raw, versioned):
  data/confirmatory_PRIVATE/analysis/judge_scores_corrected.jsonl      (8,244 cells: code/panel/original, one rule)
...
# Pre-registered original 15-country set (everything else is the extension).
...
# Load raw data
# ---------------------------------------------------------------------------
# BASE UNICA (2026-09-21): as tabelas do suplemento leem a MESMA base que o
# corpo — judge_scores_corrected.jsonl (celula deduplicada, T1/T2/T3 por codigo,
# painel de 3 no residuo, EGY sem chave em T1 excluida). Antes liam a pontuacao
# original de 9.251 linhas, e a Tabela S1 discordava da Tabela 2 em todos os
# modelos (DeepSeek 0,625 contra 0,680). A auditoria externa pegou.
...
JUDGE = "code (T1; part of T2, T3) / three-vendor panel (rest of T2, T3; T4) / collection judge (T5)"
N_TOTAL = len(rows)
...
lines = [
 ...
 r"\caption{Deployment-stack roster. Each model is evaluated \emph{as served} through a fixed access stack; the API/tag string and execution venue are part of what each label denotes. Parameter counts are vendor-reported (dense or total for MoE; closed-frontier counts undisclosed). $N$ and mean are realized English-prompt coverage.\label{tab:s-roster}}\\",
 r"\toprule",
...
for mid,(disp,vendor,tier,api,venue,p) in sorted(
    ...
    lines.append("%s & %s & %s & \\texttt{%s} & %s & %d & %.3f \\\\" % (
        tex_escape(disp), vendor, tier, tex_escape(api).replace("/", "/\\allowbreak "), venue, len(vals), statistics.mean(vals)))
...
lines = [
 ...
 r"\caption{Coverage matrix: realized English-prompt responses per model $\times$ task (T1 standard, T2 local datum, T3 health synthesis, T4 instruments, T5 recommendation). No cell is silently dropped; shortfalls are quota-driven (Methods).\label{tab:s-cov-task}}\\",
 r"\toprule",
...
lines = [
 ...
 r"\caption{Coverage by country: tier (GN/GS), sample set (P = pre-specified 15; E = extension), and realized responses (English, native-language, total).\label{tab:s-cov-country}}\\",
 r"\toprule",
...
lines = [
 ...
 r"\caption{Native-language responses by language and country (H2 within-country English/native contrast).\label{tab:s-native}}\\",
...
lines = [
 r"% AUTO-GENERATED by code/analysis/make_supplement_tables.py",
 r"\footnotesize", 
 r"\begin{longtable}{p{1.9cm}p{4.3cm}p{1.9cm}p{4.6cm}}",
 r"\caption{Ground-truth registry for T1 (binding annual PM\textsubscript{2.5} standard), all 25 countries, anchored to official primary sources by documentary audit. All entries are of source type \emph{official primary} unless noted. Status \emph{verified} = value read from the official text; \emph{no standard} = official confirmation that no national standard exists; \emph{partial} = the value could not be confirmed in an accessible official source (Egypt only: no value extracted, excluded from T1; Bangladesh is verified against the 2022 Rules). Full URLs and verbatim excerpts are in the released registry JSONL.\label{tab:s-groundtruth}}\\",
 r"\toprule",
...
for iso in sorted(gt_by, key=lambda k: NAMES.get(k,k)):
    ...
    lines.append("%s & %s & %s & %s \\\\" % (
        ...
        {"VERIFIED":"verified","VERIFIED_NO_STANDARD":"no standard","PARTIALLY_VERIFIED":"partial"}.get(g.get("status",""), tex_escape(g.get("status",""))),
...
acq_lines = [
 ...
 r"\caption{Per-provider acquisition window and official API documentation. Reported accuracies are point-in-time estimates relative to this window; first/last are the earliest and latest non-error call timestamps (UTC) recorded in the run logs. The window spans the pilot, the pre-specified 15-country collection, and the extension.\label{tab:s-acquisition}}\\",
 r"\toprule",
