...
PROIBIDOS_SEMPRE = [
    ...
    # rodada 14 (parecer de painel, 2a): enquadramentos que a inferencia por pais derrubou
    (r"pooled odds ratio|razão de chances agrupada", "OR agrupada ignora o cluster", "IC bootstrap por pais"),
    (r"Two of three tests|Dois de três testes", "contagem de votos", "declarar cada especificacao"),
    (r"weakest of all fourteen systems|mais fraco dos catorze sistemas", "H3 como 'pior dos 14'", "sem ganho sobre a propria base"),
    (r"worst performer of all fourteen|pior performer dos catorze", "H3 como 'pior dos 14'", "sem ganho sobre a propria base"),
    (r"the gap disappears entirely|a lacuna desaparece inteiramente", "T5 e teto, nao ausencia de lacuna", "no teto nos dois tiers"),
    (r"coverage-or-development question as open|questão cobertura-ou-desenvolvimento como aberta", "IDH supera o limiar por pais", "escolha entre as exposicoes em aberto"),
    (r"survive every robustness attack|sobrevivem a todo ataque", "subconjunto balanceado nao sobrevive", "cinco de sete, marginal em dois"),
]
...
def ancoras(c: dict) -> list[tuple[str, str, tuple[str, ...]]]:
    ...
    return [
        ...
        ("H4 IDH sozinho",        Mx.get("nhfourhdialonebeta", "+0.036").lstrip("+"), RES_DISC),
        ("H2 sem as vazias",      Mx.get("nhtwonoempty", "-5.15").lstrip("-"), RES),
        ("Cabra vs base",         Mx.get("ncabsanitybase", "50"),           RES_DISC),
        ("H4 p no nivel do pais", "p=" + Mx.get("nhfourctryp", "0.069"), RES_DISC),
        ("OR bruta de T1 (pais)", Mx.get("norrawone", "0.44"),           RES),
        ("IC de Fisher do rho",   Mx.get("nrhocihi", "+0.66"),           RES_DISC),
        ("H5",                    fmt(c["h5_pp"], 1),                   RES),
        ("H6 DiD",                fmt(c["h6_did"], 1, True),            RES),
        ("p da persona",          "p=" + fmt(c["persona_p"], 2),        RES_DISC),
        ("H3 delta",              fmt(c["cliff_regional"], 2),          RES),
