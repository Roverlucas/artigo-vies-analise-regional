...
| **H1** (tier) | Global North/South gap | **+5.0 pp** (bootstrap 95% CI [+1.6,+8.4]; permutation p=0.019); same size within the extension wave alone (+5.9) and on the pre-specified 15 (+6.1, 3 GN). On the code-adjudicated outcomes alone the country-level gap is +14.7 pp (perm p=0.012), so it is not a judge artefact. Concentrated: on the binding national standard, scored by code, Global South odds are **0.32** [0.27,0.39] of Global North odds (0.39 without the three no-standard countries; 0.14 under a ladder-tolerant key); no gap on the one task with no register value | Supported, but only under the conventional UNCTAD partition — partitions on HDI alone do not reproduce it |
...
| **H4** | Corpus representation is the mechanism | Between countries, neither channel separates from development (sitelinks ρ=+0.37, p=0.070; partial p=0.37). Within countries, coverage predicts the specificity deficit (β=+0.031, p=6e-10) and the language-corpus channel does not (β=+0.004, p=0.41); but coverage and HDI correlate at ρ=0.65, and with HDI×dependence in the same model HDI survives (β=+0.027, p=5e-5) and coverage does not (β=+0.013, p=0.057) | **Identified only up to the pair coverage-or-development.** Rules out the language channel; not causal |
